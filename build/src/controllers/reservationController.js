...
exports.getReservations = exports.deleteReservation = exports.updateReservation = exports.createReservation = void 0;
...
const updateReservation = async (req, res) => {
    try {
        const { customerId, serviceId, date } = req.body;
        const reservation = await Reservation_1.default.findByIdAndUpdate(req.params.id, {
            customer: customerId,
            service: serviceId,
            date: new Date(date),
        }, { new: true });
...
const deleteReservation = async (req, res) => {
    try {
        const reservation = await Reservation_1.default.findByIdAndDelete(req.params.id);
        if (!reservation) {
            return res.status(404).json({ message: 'Reserva no encontrada' });
        }
...
const getReservations = async (req, res) => {
    try {
        ...
        const reservations = await Reservation_1.default.find(filters).populate('customer service');
